...
class Ldap():
    ...
    def get_groups(self):
        ...
        try:
            ...
            gps = conn.search_ext_s(self._get_str_ou(CN_TYPES.GROUP), ldap.SCOPE_ONELEVEL)
    ...
    def rem_group_user(self, cn, group):
        ...
        try:
            ...
            self.logger.info("LDAP - %s removeu o usuário %s do  grupo %s com sucesso." % ( self.user, cn, group ) )

        except LDAPNotFoundError, e:
            raise e
        except Exception, e:
            self.logger.error("LDAP - Erro quando %s removia o usuário %s do  grupo %s na base ldap." % ( self.user, cn, group ) )
            raise LDAPError(e)
    ...
    def get_users(self, lock=False, policy=False):
        ...
        try:
            ...
            us = conn.search_ext_s(self._get_str_ou(CN_TYPES.USER), ldap.SCOPE_ONELEVEL)        
    ...
    def get_sudoers(self):
        ...
        try:
            ...
            sds = conn.search_ext_s(self._get_str_ou(CN_TYPES.SUDOERS), ldap.SCOPE_ONELEVEL)
    ...
    def add_sudoer(self, cn, sudoHost, sudoUser = None, sudoCommand = None):
        ...
        try:
            ...
            self.logger.info("LDAP - %s criou a regra de sudo %s com sucesso." % ( self.user, cn ) )
            
        except Exception, e:
            self.logger.error("LDAP - Erro quando %s adicionava a regra de sudo %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
        finally:
            try:
                conn.unbind_s()
    ...
    def edit_sudoer(self, cn, sudoHost, sudoUser = None, sudoCommand = None):
        ...
        try:
            ...
            self.logger.info("LDAP - %s alterou a regra de sudo  %s com sucesso." % ( self.user, cn ) )
            
        except LDAPNotFoundError, e:
            raise e
        except Exception, e:
            self.logger.error("LDAP - Erro quando %s alterava a regra de sudo  %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
    ...
    def rem_sudoer(self, cn):
        ...
        try:
            ...
            self.logger.info("LDAP - %s removeu a regra de sudo %s com sucesso." % ( self.user, cn ) )

        except LDAPNotFoundError, e:
            raise e
        except Exception, e:
            self.logger.error("LDAP - Erro quando %s removia a regra de sudo %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
    ...
    def get_policies(self):
        ...
        try:
            ...
            pols = conn.search_ext_s(self._get_str_ou(CN_TYPES.POLICY), ldap.SCOPE_ONELEVEL)
    ...
    def add_user(self, cn, uidNumber, groupPattern, homeDirectory, givenName, initials, sn, mail, homePhone, mobile, street, description, employeeNumber, employeeType, loginShell, shadowLastChange, shadowMin, shadowMax, shadowWarning, policy, groups):
        ...
        try:
            ...
            # A dict to help build the "body" of the object
            attrs = {}
            attrs['objectclass'] = ['posixAccount','inetOrgPerson','top','shadowAccount']
            attrs['cn'] = cn
            attrs['uid'] = cn
            ...
            self.logger.info("LDAP - %s adicionou o usuario %s a base Ldap." % (self.user ,cn))
            ...
            self.logger.error("LDAP - Erro quando %s adicionava o usuário %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
        finally:
            try:
                conn.unbind_s()
    ...
    def edit_user(self, cn, uidNumber, groupPattern, homeDirectory, givenName, initials, sn, mail, homePhone, mobile, street, description, employeeNumber, employeeType, loginShell, shadowLastChange, shadowMin, shadowMax, shadowWarning, policy, groups):
        ...
        try:
            ...
            self.logger.info("LDAP - %s alterou o usuario %s a base Ldap." % (self.user ,cn))
            ...
            self.logger.error("LDAP - Erro quando %s alterava o usuário %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
    ...
    def _add_user_in_group(self, user, group):
        ...
        try:
            ...
            self.logger.error("LDAP - Erro quando %s adicionava o usuário %s no grupo %s na base ldap." % ( self.user, user, group ) )
            raise LDAPError(e)
    ...
    def rem_user(self, cn):
        ...
        try:
            ...
            self.logger.info("LDAP - %s removeu o usuário %s com sucesso." % ( self.user, cn ) )
            ...
            self.logger.error("LDAP - Erro quando %s removia usuário %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
    ...
    def reset_pwd(self, cn):
        ...
        try:
            ...
            self.logger.info("LDAP - %s resetou a senha do usuario %s com sucesso." % ( self.user, cn ) )
            ...
            if cmd1[0] != 0:
                self.logger.warning("LDAP - Retorno do comando para unlock de chave ssh(ldapResetPwd): %s - %s" % (cmd1[0],cmd1[1]) )
    ...
    def lock_user(self, cn):
        ...
        try:
            ...
            if cmd1[0] != 0:
                self.logger.warning("LDAP - Retorno do comando para lock de chave ssh(ldapLockUser): %s - %s" % (cmd1[0],cmd1[1]) )
            
            mod_attrs = [(ldap.MOD_REPLACE, "pwdAccountLockedTime", "000001010000Z"),(ldap.MOD_REPLACE, "nsAccountLock", "TRUE")]
            conn.modify_ext_s(dn, mod_attrs)
            
            self.logger.info("LDAP - %s bloqueou o usuário %s com sucesso." % ( self.user, cn ) )

        except LDAPNotFoundError, e:
            raise e
        except Exception, e:
            self.logger.error("LDAP - Erro quando %s bloqueava o usuário  %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)
    ...
    def unlock_user(self, cn):
        ...
        try:
            ...
            cmd1 = commands.getstatusoutput(COMMAND_TYPES.LOCK % home)
            if cmd1[0] != 0:
                self.logger.warning("LDAP - Retorno do comando para unlock de chave ssh(ldapUnlockUser): %s - %s" % (cmd1[0],cmd1[1]) )
            
            lock = self._cmd_line(cn, "pwdAccountLockedTime")
            
            mod_attrs = [(ldap.MOD_DELETE, "pwdAccountLockedTime", lock["pwdAccountLockedTime"]),(ldap.MOD_REPLACE, "nsAccountLock", "FALSE")]
            conn.modify_ext_s(dn, mod_attrs)
            
            self.logger.info("LDAP - %s desbloqueou o usuário %s com sucesso." % ( self.user, cn ) )

        except LDAPNotFoundError, e:
            raise e
        except Exception, e:
            self.logger.error("LDAP - Erro quando %s desbloqueava o usuário  %s na base ldap." % ( self.user, cn ) )
            raise LDAPError(e)