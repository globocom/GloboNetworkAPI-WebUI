...
class LDAPInvalidParameterError(LDAPError):
    ...
    def __init__(self, error, param=None, value=None):
        LDAPError.__init__(
            self, u"Invalid parameter: %s. Value: %s." %
            (param, value))
        self.param = param
        self.value = value
...
class Ldap():
    ...
    def _get_conn(self):
        ...
        try:
            ...
            conn = ldap.initialize(
                "%s://%s" %
                (('ldaps' if LDAP_SSL else 'ldap'),
                 (LDAP_INITIALIZE_SSL if LDAP_SSL else LDAP_INITIALIZE)))

            conn.protocol_version = ldap.VERSION3
            conn.simple_bind_s(
                self._get_str(
                    LDAP_CREDENTIALS_USER,
                    CN_TYPES.USER),
        ...
        try:
            ...
            for k in ob[1]:
                ...
                if cn_type == CN_TYPES.SUDOERS:
                    if len(
                            ob[1][k]) == 1 and k != "sudoCommand" and k != "sudoUser":
                        dic[k] = ob[1][k][0]
                    else:
                        dic[k] = ob[1][k]
                else:
    ...
    def _parse_sudoers(self, sudoUser, sudoCommand):
        ...
        if sudoCommand:
            ...
            if isinstance(sudoCommand, type([])):
    ...
    def _alter_cn(self, cn, cn_type, dicNew):
        ...
        try:
            ...
            "Policy"
            if cn_type == CN_TYPES.USER and "pwdPolicySubentry" in dicOld:
                ...
                if dicOld["pwdPolicySubentry"] != dicNew["pwdPolicySubentry"]:
                    ...
                    mod_attrs = [
                        (ldap.MOD_REPLACE,
                         "pwdPolicySubentry",
                         dicNew["pwdPolicySubentry"])]

                    try:
                        conn.modify_ext_s(dn, mod_attrs)
                    except Exception as e:
                        self.logger.error(
                            "LDAP - Erro quando %s modificava politica do usuarios na base ldap:" %
                            self.user)
    ...
    def rem_group(self, cn):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s removeu o grupo %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s removia grupo %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
    ...
    def rem_group_user(self, cn, group):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s removeu o usuário %s do  grupo %s com sucesso." %
                (self.user, cn, group))
            ...
            self.logger.error(
                "LDAP - Erro quando %s removia o usuário %s do  grupo %s na base ldap." %
                (self.user, cn, group))
            raise LDAPError(e)
    ...
    def add_group(self, cn, gid_number, member_uid=None):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s criou o grupo %s com sucesso." %
                (self.user, cn))

        except Exception as e:
            self.logger.error(
                "LDAP - Erro quando %s adicionava grupo %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
        finally:
            try:
    ...
    def edit_group(self, cn, gid_number, member_uid=None):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s alterou o grupo %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s alterava grupo %s na base ldap." %
                (self.user, cn))
            raise LDAPMethodError(e)
                ...
                if lock:
                    ...
                    if not lockTime == "":
                        if lockTime == "000001010000Z":
                            ...
                            lockTimeArray = [
                                lockTimeData.strftime("%Y"),
                                lockTimeData.strftime("%m"),
                                lockTimeData.strftime("%d"),
                                lockTimeData.strftime("%H"),
                                lockTimeData.strftime("%M"),
                                lockTimeData.strftime("%S")]
    ...
    def get_sudoers(self):
        ...
        try:
            ...
            for sud in sds:
                ...
                for k in sud[1]:
                    if len(
                            sud[1][k]) == 1 and k != "sudoCommand" and k != "sudoUser":
                        dic[k] = sud[1][k][0]
                    else:
                        dic[k] = sud[1][k]
    ...
    def add_sudoer(self, cn, sudoHost, sudoUser=None, sudoCommand=None):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s criou a regra de sudo %s com sucesso." %
                (self.user, cn))

        except Exception as e:
            self.logger.error(
                "LDAP - Erro quando %s adicionava a regra de sudo %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
        finally:
            try:
    ...
    def edit_sudoer(self, cn, sudoHost, sudoUser=None, sudoCommand=None):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s alterou a regra de sudo  %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s alterava a regra de sudo  %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
    ...
    def rem_sudoer(self, cn):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s removeu a regra de sudo %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s removia a regra de sudo %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
            ...
            if "memberUid" in gp:
                ...
                for ldap_usr in out:
                    try:
                        ...
                        if usr['givenName'] is None or usr['givenName'] == '' or (
                                usr.get('nsaccountlock') is not None and usr['nsaccountlock'] == 'TRUE'):
                            out.remove(ldap_usr)
                    except Exception:
                        out.remove(ldap_usr)
    ...
    def add_user(
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s adicionou o usuario %s a base Ldap." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s adicionava o usuário %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
        finally:
            try:
    ...
    def edit_user(
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s alterou o usuario %s a base Ldap." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s alterava o usuário %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
    ...
    def _add_user_in_group(self, user, group):
        ...
        try:
            ...
            self.logger.error(
                "LDAP - Erro quando %s adicionava o usuário %s no grupo %s na base ldap." %
    ...
    def rem_user(self, cn):
        ...
        try:
            ...
            self.logger.info(
                "LDAP - %s removeu o usuário %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s removia usuário %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
    ...
    def reset_pwd(self, cn):
        ...
        try:
            ...
            mod_attrs = [
                (ldap.MOD_REPLACE,
                 "userPassword",
                 LDAP_PASSWORD_DEFAULT_HASH),
                (ldap.MOD_REPLACE,
                 "shadowLastChange",
                 "1")]  # senha globocom em md5

            conn.modify_ext_s(dn, mod_attrs)

            self.logger.info(
                "LDAP - %s resetou a senha do usuario %s com sucesso." %
                (self.user, cn))
            ...
            if cmd1[0] != 0:
                self.logger.warning(
                    "LDAP - Retorno do comando para unlock de chave ssh(ldapResetPwd): %s - %s" %
                    (cmd1[0], cmd1[1]))
    ...
    def lock_user(self, cn):
        ...
        try:
            ...
            if cmd1[0] != 0:
                self.logger.warning(
                    "LDAP - Retorno do comando para lock de chave ssh(ldapLockUser): %s - %s" %
                    (cmd1[0], cmd1[1]))
            ...
            self.logger.info(
                "LDAP - %s bloqueou o usuário %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s bloqueava o usuário  %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
    ...
    def unlock_user(self, cn):
        ...
        try:
            ...
            if cmd1[0] != 0:
                self.logger.warning(
                    "LDAP - Retorno do comando para unlock de chave ssh(ldapUnlockUser): %s - %s" %
                    (cmd1[0], cmd1[1]))
            ...
            self.logger.info(
                "LDAP - %s desbloqueou o usuário %s com sucesso." %
                (self.user, cn))
            ...
            self.logger.error(
                "LDAP - Erro quando %s desbloqueava o usuário  %s na base ldap." %
                (self.user, cn))
            raise LDAPError(e)
