...
def ajax_shared_view_vip(request, id_vip, lists):
    ...
    try:
        ...
        if 'reals' in lists['vip']:
            if not isinstance(lists['vip']['reals']['real'], list):
        ...
        if 'portas_servicos' in lists['vip']:
            if not isinstance(lists['vip']['portas_servicos']['porta'], list):
...
def delete_validate_create_remove(request, operation):
    ...
    if request.method == 'POST':
        ...
        form = DeleteForm(
            request.POST) if operation_text == 'DELETE' else ValidateForm(
            request.POST) if operation_text == 'VALIDATE' else CreateForm(
            request.POST) if operation_text == 'CREATE' else RemoveForm(
                request.POST)
        ...
        if form.is_valid():
            ...
            for id_vip in ids:
                ...
                except VipAllreadyCreateError as e:
                    logger.error(e)
            ...
            if len(error_list) == len(ids):
                messages.add_message(
                    request,
                    messages.ERROR,
                    request_vip_messages.get(msg_error_call))
            ...
            # If all has ben removed
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    request_vip_messages.get(msg_sucess_call))
...
def valid_reals(
    ...
    if id_equip is not None and id_ip is not None:
        ...
        invalid_port_vip = [
            i for i in ports_vip_reals if int(i) > 65535 or int(i) < 1]
        invalid_ports_real = [
            i for i in ports_real_reals if int(i) > 65535 or int(i) < 1]
        invalid_priority = [
            i for i in priority if int(i) > 4294967295 or int(i) < 0]

        if invalid_priority:
            lists['reals_error'] = request_vip_messages.get("invalid_priority")
            is_valid = False
        ...
        if len(ports_vip_reals) != len(ports_real_reals):
            ...
            duplicated = 0
            for i in range(len(ports_real_reals)):
                temp_real = ports_real_reals[i]
                temp_ip = id_ip[i]
                if duplicated > 1:
                    lists['reals_error'] = request_vip_messages.get(
                        "duplicated_value")
                    is_valid = False
                else:
                    duplicated = 0
                    for j in range(len(ports_real_reals)):
                        if temp_real == ports_real_reals[
                                j] and temp_ip == id_ip[j]:
                            duplicated += 1

        if str(balancing).upper() == "WEIGHTED":

            invalid_weight = [
                i for i in weight if int(i) > 4294967295 or int(i) < 0]
            if invalid_weight:
                lists['reals_error'] = request_vip_messages.get(
                    "invalid_weight")
                is_valid = False
                        ...
                        reals.append(
                            {
                                ...
                                'id_ip': id_ip[i],
                                'ip': ip[i],
                                'ports_vip': ports_vip_reals[i],
                                'ports_real': ports_real_reals[i]})

                    else:
                        if not weight and not priority:
                            reals.append({'priority': '',
                                          'weight': '',
                                          'id_equip': id_equip[i],
                                          'equip': equip[i],
                                          'id_ip': id_ip[i],
...
def reals_(id_equip, id_ip, equip, ip, ports_vip_reals, ports_real_reals):
    ...
    if id_equip is not None and id_ip is not None:
        ...
        reals = []
        for i in range(0, len(id_equip)):
            reals.append({"real_name": equip[i],
                          "real_ip": ip[i],
                          "port_vip": ports_vip_reals[i],
                          "id_ip": id_ip[i],
                          "port_real": ports_real_reals[i]})
...
def valid_form_and_submit(
    ...
    #Real - data
    ports_vip = valid_field_table_dynamic(
        request.POST.getlist('ports_vip')) if "ports_vip" in request.POST else None
    ports_real = valid_field_table_dynamic(
        request.POST.getlist('ports_real')) if "ports_real" in request.POST else None
    ...
    if form_inputs.is_valid() & form_environment.is_valid() & form_real.is_valid(
    ) & form_healthcheck.is_valid() & form_options.is_valid() & form_ip.is_valid():
    ...
    #Real - data
    ports_vip = request.POST.getlist(
        'ports_vip') if "ports_vip" in request.POST else None
    ports_real = request.POST.getlist(
        'ports_real') if "ports_real" in request.POST else None

    lists = mount_table_ports(lists, ports_vip, ports_real)

    #Real - data
    priority = request.POST.getlist(
        'priority') if "priority" in request.POST else None
    weight = request.POST.getlist(
        'weight') if "weight" in request.POST else None
    id_equip = request.POST.getlist(
        'id_equip') if "id_equip" in request.POST else None
    ...
    # Real - data TABLE REALS
    ports_vip_reals = request.POST.getlist(
        'ports_vip_reals') if "ports_vip_reals" in request.POST else None
    ports_real_reals = request.POST.getlist(
        'ports_real_reals') if "ports_real_reals" in request.POST else None
...
def parse_real_server(
    ...
    for i in range(0, len(reals)):
        ...
        except (EquipamentoNaoExisteError, IpNaoExisteError, IpNotFoundByEquipAndVipError) as e:
            logger.error(e)
            messages.add_message(
                request,
                messages.ERROR,
                request_vip_messages.get("error_existing_reals") %
                (real_name,
                 real_ip))
...
def tab_real_server_status(request, id_vip):
    ...
    try:
        ...
        id_evip = client_api.create_environment_vip().search(
            ...
            finality,
            client,
            environment).get("environment_vip").get("id")
        ...
        if 'reals' in vip:
            ...
            if not isinstance(reals_list, list):
        ...
        messages.add_message(
            request,
            messages.ERROR,
            request_vip_messages.get("error_existing_environment_vip") %
            (finality,
             client,
             environment))
    ...
    return render_to_response(
        VIPREQUEST_TAB_REAL_SERVER_STATUS,
        lists,
        context_instance=RequestContext(request))
    ...
    try:
            ...
            try:
                ...
                if is_valid_reals:
                    ...
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        request_vip_messages.get("tab_edit_success") %
                        'real server')
                else:
                    html = lists['reals_error']
                    parse = False

            except (EnvironmentVipError, InvalidTimeoutValueError, InvalidBalMethodValueError, InvalidCacheValueError, InvalidPersistenceValueError, InvalidPriorityValueError, EquipamentoNaoExisteError, IpEquipmentError, IpError, RealServerPriorityError, RealServerWeightError, RealServerPortError, RealParameterValueError) as e:
                logger.error(e)
                messages.add_message(
                    request,
                    messages.ERROR,
                    request_vip_messages.get("tab_edit_error") %
                    'real server')

                try:
                    raise e
                except InvalidTimeoutValueError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        request_vip_messages.get("error_existing_timeout") %
                        vip['timeout'])
                except InvalidBalMethodValueError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        request_vip_messages.get("error_existing_balancing") %
                        vip['balancing'])
                except InvalidCacheValueError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        request_vip_messages.get("error_existing_cache") %
                        vip['cache'])
                except InvalidPersistenceValueError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        request_vip_messages.get("error_existing_persistence") %
                        vip['persistencia'])
                except InvalidPriorityValueError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        request_vip_messages.get("error_priority"))
                except EnvironmentVipError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        request_vip_messages.get("error_existing_environment_vip") %
                        (finality,
                         client,
                         environment))
                except (EquipamentoNaoExisteError, IpEquipmentError, IpError, RealServerPriorityError, RealServerWeightError, RealServerPortError, RealParameterValueError) as e:
                    messages.add_message(request, messages.ERROR, e)

            except RealServerScriptError as e:
                logger.error(e)
        ...
        if parse is True:
            ...
            if "portas_servicos" in vip:
                if not isinstance(vip['portas_servicos'], NoneType):
                    if isinstance(
                            vip['portas_servicos']['porta'],
                            unicode) or len(
                            vip['portas_servicos']['porta']) == 1:
                        vip['portas_servicos']['porta'] = [
                            vip['portas_servicos']['porta']]
    ...
    try:
        ...
        try:
            environment_vip = client_api.create_environment_vip().search(
                ...
                vip['finalidade'],
                vip['cliente'],
                vip['ambiente']).get("environment_vip").get("id")
            rules = client_api.create_option_vip().buscar_rules(
                environment_vip,
                id_vip).get('name_rule_opt')
            ...
            if form_real.is_valid() and form_rules.is_valid():
                ...
                try:
                    ...
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        request_vip_messages.get('success_l7_save'))
...
def status_real_server(request, id_vip, status):
        ...
        if form.is_valid():
            ...
            if len(error_list) == len(ids):
                messages.add_message(
                    request,
                    messages.ERROR,
                    request_vip_messages.get("can_not_real_all") %
                    ('Habilitar' if enable else 'Desabilitar'))
            ...
            # If cant change someones
                ...
                messages.add_message(
                    request, messages.WARNING, request_vip_messages.get("can_not_real") %
                    (('Habilitar' if enable else 'Desabilitar'), msg[
                        :-2]))
            ...
            # If all has ben changed
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    request_vip_messages.get("success_real") %
                    ('Habilitado' if enable else 'Desabilitado'))
    ...
    return HttpResponseRedirect(
        reverse(
            'vip-request.tab.real.server.status.edit',
            args=[id_vip]))
...
def generate_token(request):
    try:
        ...
        form = GenerateTokenForm(
            request.POST) if request.method == 'POST' else GenerateTokenForm()
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['is_ldap_user']:
                ...
                if pwd == pwd_ldap and (
                        activate is None or activate.upper() == 'FALSE'):
                    # Valid User
            ...
            if client_user is None:
                ...
                if idt is not None and not is_valid_int_param(idt):
                    raise Exception(
                        error_messages.get("invalid_param") %
                        "requestVip")
                ...
                lists["token"] = token
                lists["url"] = reverse(
                    "vip-request.edit.external",
                    args=[idt]) if idt is not None else reverse("vip-request.form.external")
...
def edit_form_shared(
    ...
    try:
        ...
        if request.method == "POST":
            ...
            form_inputs = RequestVipFormInputs(
                initial={
                    "business": business,
                    "service": service,
                    "name": name,
            ...
            if id_healthcheck_expect is not None:
                excpect = client_api.create_ambiente().buscar_healthcheck_por_id(
                    id_healthcheck_expect).get("healthcheck_expect").get("id")

            client_ovip = client_api.create_option_vip()
            healthcheck_options = client_ovip.buscar_healthchecks(
                int(environment_vip))
            healthcheck_options = healthcheck_options.get(
                'healthcheck_opt',
                [])
...
def popular_client_shared(request, client_api):
    ...
    try:
        ...
        lists['clients'] = validates_dict(
            client_evip.buscar_cliente_por_finalidade(finality),
            'cliente_txt')
...
def popular_environment_shared(request, client_api):
    ...
    try:
        ...
        lists['environments'] = validates_dict(
            client_evip.buscar_ambientep44_por_finalidade_cliente(
                finality,
                client),
            'ambiente_p44')
...
def popular_options_shared(request, client_api):
    ...
    try:
        ...
        lists['timeout'] = validates_dict(
            client_ovip.buscar_timeout_opcvip(environment_vip),
            'timeout_opt')
        lists['balancing'] = validates_dict(
            client_ovip.buscar_balanceamento_opcvip(environment_vip),
            'balanceamento_opt')
        lists['caches'] = validates_dict(
            client_ovip.buscar_grupo_cache_opcvip(environment_vip),
            'grupocache_opt')
        lists['persistence'] = validates_dict(
            client_ovip.buscar_persistencia_opcvip(environment_vip),
            'persistencia_opt')
        lists['rules'] = validates_dict(
            client_ovip.buscar_rules(
                environment_vip,
                id_vip),
            'name_rule_opt')
...
def model_ip_real_server_shared(request, client_api):
    ...
    try:
        ...
        equip_real = split_to_array(
            get_param_in_request(
                request,
                'equip_real')) if get_param_in_request(
            request,
            'equip_real') is not None else None
        ips_real = split_to_array(
            get_param_in_request(
                request,
                'ips_real')) if get_param_in_request(
            request,
            'ips_real') is not None else None
    ...
    except EquipamentoNaoExisteError as e:
        logger.error(e)
    ...
    lists['ips'] = loader.render_to_string(
        AJAX_VIPREQUEST_MODEL_IP_REAL_SERVER_HTML, {
            'ips': ips, 'equip': equip, 'balancing': balancing}, context_instance=RequestContext(request))

    # Returns Json
    return HttpResponse(
        loader.render_to_string(
            AJAX_VIPREQUEST_MODEL_IP_REAL_SERVER,
            lists,
            context_instance=RequestContext(request)),
        status=status_code)
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    request_vip_messages.get('success_l7_validate'))
            except Exception as e:
                raise Exception(e)
...
@login_required
@has_perm([{"permission": VIP_ALTER_SCRIPT, "read": True},
           {"permission": VIP_ALTER_SCRIPT, "write": True}])
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    request_vip_messages.get('success_l7_alter'))
            except Exception as e:
                messages.add_message(request, messages.ERROR, e)
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    request_vip_messages.get('success_l7_rollback'))
            except Exception as e:
                messages.add_message(request, messages.ERROR, e)
