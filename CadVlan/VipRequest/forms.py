...
class RequestVipFormOptions(forms.Form):
        ...
        if environment_vip is not None and environment_vip != '':
            ...
            if persistences is not None:
                self.fields['persistence'].choices = [
                    (st['persistencia_opt'],
                     st['persistencia_opt']) for st in persistences]
            else:
                if 'initial' in kwargs:
            ...
            if balancings is not None:
                self.fields['balancing'].choices = [
                    (st['balanceamento_opt'],
                     st['balanceamento_opt']) for st in balancings]
            else:
                if 'initial' in kwargs:
            ...
            if rules is not None:
                self.fields['rules'].choices = [
                    (st['id'] if st['id'] is not None else '',
                     st['name_rule_opt'] if st['name_rule_opt'] is not None else '') for st in rules]
            else:
                if 'initial' in kwargs:
    ...
    balancing = forms.ChoiceField(
        label=u'Método de balanceamento',
        required=True,
        error_messages=error_messages,
        widget=forms.Select(
            attrs={
                "style": "width: 300px"}))
    rules = forms.ChoiceField(
        label=u'Regras',
        required=False,
        error_messages=error_messages,
        widget=forms.Select(
...
class RequestVipFormHealthcheck(forms.Form):
    ...
    def __init__(self, healthcheck_list, healthcheck_options, *args, **kwargs):
        ...
        self.fields['healthcheck_type'].choices = [
            (healthcheck['name'],
             healthcheck['name']) for healthcheck in healthcheck_options]
...
class FilterL7Form(forms.Form):
    filter_applied = forms.CharField(
        label=u'Filtro L7 atual',
        required=False,
        error_messages=error_messages,
        widget=forms.Textarea(
            attrs={
                ...
                'disabled': 'disabled'}))
    filter_l7 = forms.CharField(
        label=u'Novo Filtro L7 a ser aplicado',
        required=False,
        error_messages=error_messages,
        widget=forms.Textarea(
            attrs={
                'style': "width: 300px",
                'rows': 10}))
    filter_rollback = forms.CharField(
        label=u'Rollback (última configuração válida)',
        required=False,
        error_messages=error_messages,
        widget=forms.Textarea(
            attrs={
