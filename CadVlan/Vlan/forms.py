...
class SearchVlanForm(forms.Form):
    ...
    acl = forms.BooleanField(
        label="Apenas Acls não validadas",
        required=False,
        error_messages=error_messages)
...
class VlanForm(forms.Form):
    ...
    number = forms.IntegerField(
        ...
        widget=forms.TextInput(
            attrs={
                ...
                "maxlength": "9"}))
    environment = forms.ChoiceField(
        label="Ambiente", required=True, choices=[
            (0, "Selecione")], error_messages=error_messages, widget=forms.Select(
            attrs={
                "style": "width: 400px"}))
    acl_file = forms.CharField(
        label="Acl - IPv4",
        required=False,
        min_length=3,
        max_length=200,
    ...
    network_ipv4 = forms.ChoiceField(
        label='Adicionar rede IPv4 automaticamente',
        required=False,
        choices=CHOICES_NETWORK,
        widget=forms.Select())
    network_ipv6 = forms.ChoiceField(
        label='Adicionar rede IPv6 automaticamente',
        required=False,
        choices=CHOICES_NETWORK,
        widget=forms.Select())
