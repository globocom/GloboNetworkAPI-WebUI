...
def search_list(request):
        ...
        if request.method == "GET":
            ...
            if request.GET.__contains__('equip_name'):
                ...
                if search_form.is_valid():
                    ...
                    if 'interfaces' in equip_interface_list:
    ...
    return render_to_response(
        EQUIPMENT_INTERFACE_SEARCH_LIST,
        lists,
...
def add_several_forms(request, equip_name):
    ...
    divisor = "/" if (brand == '3' or brand ==
                      '5') else ":" if brand == "0" else "." if brand == '4' else ""
    ...
    if int(brand) not in list_brands:
        ...
        messages.add_message(
            request,
            messages.ERROR,
            equip_interface_messages.get("brand_error"))
        url_param = reverse("equip.interface.search.list")
        if len(equip_name) > 2:
            url_param = url_param + "?equip_name=" + equip_name
        return HttpResponseRedirect(url_param)
        ...
        if form.is_valid():
            ...
            if (last_piece_name > last_piece_name2):
                messages.add_message(
                    request,
                    messages.ERROR,
                    equip_interface_messages.get("name_error"))
                return render_to_response(
                    EQUIPMENT_INTERFACE_SEVERAL_FORM,
                    lists,
                    context_instance=RequestContext(request))
            ...
            for interface_number in range(last_piece_name, last_piece_name2 + 1):
                ...
                try:
                    client.create_interface().inserir(
                        ...
                        equip.get('id'))
                    goodCont = goodCont + 1
                except NomeInterfaceDuplicadoParaEquipamentoError as e:
                    logger.error(e)
                    erro.append(name)
                    evilCont = evilCont + 1
            ...
            if not api_error:
                ...
                if cont == goodCont:
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        equip_interface_messages.get("several_sucess"))
                    url_param = reverse("equip.interface.search.list")
                    if len(equip_name) > 2:
                        url_param = url_param + "?equip_name=" + equip_name
                    return HttpResponseRedirect(url_param)
                    ...
                    messages.add_message(
                        request,
                        messages.WARNING,
                        equip_interface_messages.get("several_warning") %
                        (msg_erro))
                    if len(equip_name) > 2:
                        url_param = url_param + "?equip_name=" + equip_name
                    return HttpResponseRedirect(url_param)
                    ...
                    messages.add_message(
                        request,
                        messages.ERROR,
                        equip_interface_messages.get("several_error") %
                        (msg_erro))
            ...
            messages.add_message(
                request,
                messages.ERROR,
                equip_interface_messages.get("validation_error"))
...
def edit_form(request, equip_name, id_interface):
    ...
    AddInterfaceFormSet = formset_factory(
        AddInterfaceForm,
        params=params,
        ...
        extra=len(related_list),
        max_num=len(related_list))
    ...
    if request.method == "POST":
        ...
        if form_set.is_valid():
            ...
            for form in form_set:
                ...
                try:
                    client.create_interface().alterar(
                        idt,
                        name,
                        protected,
                        description,
                        old_interf['ligacao_front'],
                        old_interf['ligacao_back'])
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        equip_interface_messages.get("success_edit"))
                except NetworkAPIClientError as e:
                    logger.error(e)
                    messages.add_message(request, messages.ERROR, e)

            return redirect(
                "equip.interface.edit.form",
                equip_name,
                id_interface)
...
def connect(request, id_interface, front_or_back):
        ...
        if request.method == "GET":
            ...
            if request.GET.__contains__('equip_name'):
                ...
                if search_form.is_valid():
                    ...
                    lists['connect_form'] = ConnectForm(
                        equipment,
                        interf_list['interfaces'],
                        initial={
                            'equip_name': equipment['nome'],
                            'equip_id': equipment['id']})
            ...
            if form.is_valid():
                ...
                if len(front) > 0:
                    ...
                    related_list = client.create_interface().list_connections(
                        inter_front["interface"],
                        inter_front["equipamento"])
                    ...
                    # Business Rules
                    interface_client.alterar(
                        inter_front['id'],
                        inter_front['interface'],
                        inter_front['protegida'],
                        inter_front['descricao'],
                        interface['id'],
                        inter_front['ligacao_back'])
                    if front_or_back == "0":
                        interface_client.alterar(
                            interface['id'],
                            interface['interface'],
                    ...
                    interface_client.alterar(
                        inter_back['id'],
                        inter_back['interface'],
                        inter_back['protegida'],
                        inter_back['descricao'],
                        inter_back['ligacao_front'],
                        interface['id'])
                    if front_or_back == "0":
                        interface_client.alterar(
                            interface['id'],
    ...
    return render_to_response(
        EQUIPMENT_INTERFACE_CONNECT_FORM,
        lists,
        context_instance=RequestContext(request))
...
def disconnect(
    ...
    try:
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            equip_interface_messages.get("success_disconnect"))
