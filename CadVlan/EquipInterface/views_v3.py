...
def edit_interface_post(request, interface=None):
    ...
    if request.method == "POST":
        ...
        interface_dict = dict(id=int(interfaces.get('id')),
                              interface=request.POST.get('name'),
                              description=request.POST.get('description'),
                              protected=True if int(
                                  request.POST.get('protected')) else False,
                              native_vlan=request.POST.get('vlan_nativa'),
                              type=int(int_type),
                              equipment=int(equipment.get('id')),
                              front_interface=interfaces.get(
                                  'front_interface'),
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if all_interfaces:
                ...
                for i in interface_list:
                    allInt.append(
                        {'id': i.get('id'), 'interface': i.get('interface')})
...
def add_channel(request, interface_id=None):
    ...
    if interface_id:
        ...
        messages.add_message(request, messages.ERROR,
                             "Select an interface.")
        url_param = request.META.get('HTTP_REFERER') if request.META.get(
            'HTTP_REFERER') else reverse('interface.list')
...
def add_channel_(request):
    ...
    if request.method == "POST":
        ...
        try:
            channel_obj = client.create_api_interface_request().create_channel([
                channel])
            channel_id = channel_obj[0].get('id')
            messages.add_message(request, messages.SUCCESS,
                                 "O channel foi criado com sucesso!")
        except NetworkAPIClientError as e:
            logger.error(e)
            messages.add_message(request, messages.ERROR,
                                 "Erro ao criar o channel. %s" % e)
...
def dissociate_channel_interface(request, channel_id, interface_id):
    ...
    for obj in environments:
        group = dict(env=obj.get('environment').get(
            'id'), vlans=obj.get('range_vlans'))
        envs_vlans.append(group)
