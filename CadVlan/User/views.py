...
def ajax_ldap_users_by_group(request, ldap_group, id_user):
    try:
        ...
        for usr in user_list:
            if usr['user_ldap'] is not None and usr[
                    'user_ldap'] != '' and usr['id'] != id_user:
...
def list_user(client):
    ...
    for user in users:
        if user['grupos'] is not None and not isinstance(user['grupos'], list):
...
def create_form_user(id_user, auth, groups, edit, ldap_grps=None):
    ...
    if edit:
        ...
        if isinstance(user['grupos'], unicode):
...
def save_user(form, client):
    ...
    if user_ldap is None:
        ...
        send_email = EmailMessage(
            'Novo Usuário CadVlan-Globo.com',
            loader.render_to_string(
                MAIL_NEW_USER,
                list_mail),
            EMAIL_FROM,
...
def edit_user(form, client, id_user):
    ...
    groups = client.create_usuario().get_by_id(
        id_user).get('usuario')['grupos']
