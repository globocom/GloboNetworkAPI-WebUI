...
class UserForm(forms.Form):
    ...
    def __init__(self, group_list, ldap_group_list=None, ldap_user_list=None, *args, **kwargs):
        ...
        if ldap_group_list is not None:
            self.fields['ldap_group'].choices = [(ldap_grp[0], ldap_grp[1]) for ldap_grp in ldap_group_list]
            
        if ldap_user_list is not None:
            self.fields['ldap_user'].choices = [(ldap_usr, ldap_usr) for ldap_usr in ldap_user_list]
    ...
    ldap_user = forms.ChoiceField(label='Usuário LDAP', required=False, widget=forms.Select(attrs={'style': "width: 507px", 'size': "10"}))
    ...
    def clean_ldap_user(self):
        ...
        try:
            ...
            if self.cleaned_data['ldap_user'] == '':
                raise forms.ValidationError('Escolha um usuário LDAP para associar')
