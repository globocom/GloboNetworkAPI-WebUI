        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    network_ip_messages.get("success_insert"))
...
def vlan_add_network_form(request, id_vlan):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            # Forms
            lists['form'] = NetworkForm(
                net_type_list,
                env_vip_list,
                initial={
                    'vlan_name': vlan_nome,
                    'vlan_name_id': vlan.get("id")})
    ...
    try:
        ...
        if id_vip is None:
            ...
            if options_vip is not None:
                ...
                if isinstance(options_vip, dict):
            ...
            if form.is_valid():
                ...
                if not (is_valid_ipv4(ip)):
                    messages.add_message(
                        request,
                        messages.ERROR,
                        network_ip_messages.get("ip_error"))
                    lists['id'] = id_net
                    lists['form'] = form
                    lists['oct1'] = oct1
                    lists['oct2'] = oct2
                    ...
                    try:
                        ...
                        messages.add_message(
                            request,
                            messages.SUCCESS,
                            network_ip_messages.get("ip_sucess"))
                        return HttpResponseRedirect(
                            reverse(
                                'network.ip4.list.by.id',
                                args=[id_net]))
...
def edit_ip4(request, id_net, id_ip4):
    try:
                ...
                if not (is_valid_ipv4(ip)):
                    ...
                    try:
                        ...
                        messages.add_message(
                            request,
                            messages.SUCCESS,
                            network_ip_messages.get("ip_edit_sucess"))
                        return HttpResponseRedirect(
                            reverse(
                                'network.ip4.list.by.id',
                                args=[id_net]))
        ...
        if request.method == 'GET':
            ...
            nomesEquipamentos = ''
            cont = 0
            if isinstance(ip.get('equipamento'), dict):
            ...
            if form.is_valid():
                ...
                if not (is_valid_ipv6(ip6)):
                    messages.add_message(
                        request,
                        messages.ERROR,
                        network_ip_messages.get("ip6_error"))
                    lists['id_net'] = id_net
                    lists['id_ip6'] = id_ip6
                    lists['form'] = form
                    lists['equipamentos'] = equipamentos
            ...
            if form.is_valid():
                ...
                for id in ids:
                    if id not in listaIps:
                        messages.add_message(
                            request, messages.ERROR, network_ip_messages.get("not_ip_in_net") %
                            (id, id_net))
                        return HttpResponseRedirect(
                            reverse(
                                'network.ip4.list.by.id',
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    network_ip_messages.get("sucess_edit"))
    ...
    try:
        ...
        # Get
            ...
            vlan_nome = "%s | %s - %s - %s" % (vlan.get("num_vlan"),
                                               ...
                                               environment.get("nome_ambiente_logico"),
                                               environment.get("nome_grupo_l3"))
            lists['form'] = lists['form'] = NetworkEditForm(
                net_type_list,
                env_vip_list,
                initial={
                    'vlan_name': vlan_nome,
    ...
    try:
        ...
        if id_ip is None or id_ip == "":
            messages.add_message(
                request,
                messages.SUCCESS,
                network_ip_messages.get("net_invalid"))
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            network_ip_messages.get("ip_equip_delete"))

        return HttpResponseRedirect(
            reverse(
                'equipment.edit.by.id',
                args=[id_equip]))
            ...
            if form.is_valid():
                ...
                if new_equip in equipamentos.split(','):
                    messages.add_message(
                        request,
                        messages.ERROR,
                        network_ip_messages.get("already_assoc_equip"))
                    flag_error = True
                else:
