...
def add_form(request):
    ...
    try:
        ...
        logger.error(e)
        messages.add_message(
            request, messages.ERROR, equip_access_messages.get("no_equip"))
    lists['form'] = SearchEquipForm()
...
def edit(request, id_access):
    try:
            ...
            if form.is_valid():
                ...
                try:
                    client.create_equipamento_acesso().edit_by_id(
                        id_acesso, protocol, host, user, password, second_password)
                    messages.add_message(
                        request, messages.SUCCESS, equip_access_messages.get("success_edit"))
                    return search_list_param(request, equip)
        ...
        if equip_access_edit is None:
            messages.add_message(
                request, messages.ERROR, equip_access_messages.get("invalid_equip_acess"))
