            ...
            if request.GET.__contains__('equip_name'):
                ...
                if form.is_valid():
                    ...
                    if len(equip_access_list.get('equipamento_acesso')) > 0:
                        ...
                        equip_access_list = replace_id_to_name(
                            equip_access_list["equipamento_acesso"],
                    ...
                    lists['delete_form'] = DeleteForm(
                        initial={
                            "equip_name": name_equip,
                            "equip_id": id_equip})

                return render_to_response(
                    EQUIPMENTACESS_SEARCH_LIST,
                    lists,
...
def add_form(request):
    ...
    try:
            ...
            if formCad.is_valid():
                ...
                try:
                    ...
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        equip_access_messages.get("success_insert"))
                    return search_list_param(request, equip)
                except EquipamentoAcessoError as e:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        equip_access_messages.get("already_association"))
                except NetworkAPIClientError as e:
                    messages.add_message(request, messages.ERROR, e)
        ...
        messages.add_message(
            request,
            messages.ERROR,
            equip_access_messages.get("no_equip"))
    lists['form'] = SearchEquipForm()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    client.create_equipamento_acesso().edit_by_id(
                        id_acesso,
                        protocol,
                        host,
                        user,
                    ...
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        equip_access_messages.get("success_edit"))
                    return search_list_param(request, equip)

                except NetworkAPIClientError as e:
                    messages.add_message(request, messages.ERROR, e)

            equip_access = client.create_equipamento_acesso().get_access(
                id_acesso)
            lists['id_equip'] = equip_access.get(
                'equipamento_acesso').get('equipamento')
        ...
        equip_access_edit = client.create_equipamento_acesso().get_access(
            id_acesso)
        ...
        if equip_access_edit is None:
            messages.add_message(
                request,
                messages.ERROR,
                equip_access_messages.get("invalid_equip_acess"))
        ...
        form = EquipAccessForm(
            protocol_list,
            initial={
                'password': equip_access_edit.get('password'),
                    ...
                    # If all has ben removed
                        ...
                        if not flag:
                            ...
                            messages.add_message(
                                request,
                                messages.SUCCESS,
                                equip_access_messages.get("success_remove"))
                    else:
                        messages.add_message(
                            request,
                            messages.ERROR,
