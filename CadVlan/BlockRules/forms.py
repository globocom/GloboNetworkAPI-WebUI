...
class ContentRulesForm(forms.Form):
    ...
    content = forms.CharField(label=u'Conteúdo', required=True, error_messages=error_messages, widget=forms.Textarea(attrs={'style': 'width: 300px', 'rows': 7}))
    rule_content = forms.CharField(widget=forms.HiddenInput(attrs = {'order': ''}), label='', required=False, initial=0)
