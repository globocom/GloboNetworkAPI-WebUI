...
class BlockRulesForm(forms.Form):
    ...
    content = forms.CharField(label=u'Conteúdo', required=True, error_messages=error_messages, widget=forms.Textarea(
...
class EnvironmentsBlockForm(forms.Form):
    ...
    def __init__(self, env_list, *args, **kwargs):
        ...
        self.fields['envs'].choices = ([(env['id'], env["divisao_dc_name"] + " - " + env[
                                       "ambiente_logico_name"] + " - " + env["grupo_l3_name"]) for env in env_list])

    envs = forms.ChoiceField(
        label=u'Ambiente', required=True, error_messages=error_messages)


class EnvironmentRules(forms.Form):

    name = forms.CharField(label="Nome da regra", required=True,
