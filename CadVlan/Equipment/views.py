...
def remove_reals_from_equip(
    ...
    try:
        ...
        reals = [
            ...
            dict) else vip.get('reals')['real']
        priority = [
            vip.get('reals_prioritys')['reals_priority'],
        ] if isinstance(
            vip.get('reals_prioritys')['reals_priority'],
            unicode) else vip.get('reals_prioritys')['reals_priority']
        weight = [
            vip.get('reals_weights')['reals_weight'],
        ] if isinstance(
            vip.get('reals_weights')['reals_weight'],
            unicode) else vip.get('reals_weights')['reals_weight']
        ...
        if remove_all:
            ...
            for i in range(len(reals)):
                if reals[i]['real_ip'] == ip and reals[i]['port_vip'] == port_vip and reals[
                        i]['port_real'] == port_real and reals[i]['real_name'] == real_name:
                    index_to_remove.append(i)
...
@has_perm([{"permission": EQUIPMENT_MANAGEMENT,
           ...
           {"permission": ENVIRONMENT_MANAGEMENT,
            "read": True},
           {"permission": EQUIPMENT_GROUP_MANAGEMENT,
            "read": True}])
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    equip_messages.get("equip_sucess"))
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if str(orquestracao) in groups_chosen and int(
                        type_equipment) != server_virtual:
                    messages.add_message(
                        request,
                        messages.ERROR,
                        equip_messages.get("orquestracao_error"))
                    raise Exception
                ...
                environments_rm = list(
                    set(environments_list) -
                    set(environments_chosen))
                        ...
                        for grp in forms_aux['grupos']:
                            if grp["id"] == group:
                                messages.add_message(
                                    request,
                                    messages.ERROR,
                                    equip_messages.get("error_associate_group") %
                                    grp["nome"])
                                raise Exception
                                break
                        ...
                        for grp in forms_aux['grupos']:
                            if grp["id"] == group:
                                messages.add_message(
                                    request,
                                    messages.ERROR,
                                    equip_messages.get("error_disassociate_group") %
                                    grp["nome"])
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    equip_messages.get("equip_edit_sucess"))
        ...
        # GET REQUEST
            ...
            try:
                ...
                lists['form'] = EquipForm(
                    ...
                    initial={
                        "nome": equip.get('nome'),
                        "tipo_equipamento": equip.get('id_tipo_equipamento'),
                        "marca": equip.get('id_marca'),
                        "modelo": equip.get('id_modelo'),
                        "grupo": list_groups,
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    equip_messages.get("marca_sucess"))

                return render_to_response(
                    EQUIPMENT_MARCAMODELO_FORM,
                    lists,
                    context_instance=RequestContext(request))
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    equip_messages.get("modelo_sucess"))
