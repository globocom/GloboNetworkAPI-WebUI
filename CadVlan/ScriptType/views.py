...
def list_all(request):
    ...
    return render_to_response(SCRIPTTYPE_LIST, script_type_list, context_instance=RequestContext(request))
...
def show_form(request):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    client.create_tipo_roteiro().inserir(tipo.upper(), descricao)
                    ...
                    messages.add_message(request, messages.ERROR, script_type_messages.get("error_equal_name") % tipo)
    ...
    return render_to_response(SCRIPTTYPE_FORM, {'form': form}, context_instance=RequestContext(request))