        ...
        if request.method == "GET":
            ...
            if request.GET.__contains__('equip_name'):
                ...
                if search_form.is_valid():
                    ...
                    equip_script_list = client.create_equipamento_roteiro().list_by_equip(
                        name_equip)
                    ...
                    init_map = {
                        'equip_name': equipment['name'], 'equip_id': equipment['id']}

                    # New form
                    add_form = AssociateScriptForm(
                        script_list, initial=init_map)
                    ...
                    try:
                        lists['equip_script'] = equip_script_list[
                            'equipamento_roteiro']
                    except KeyError:
                        lists['equip_script'] = []
...
def delete_all(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Get user
            auth = AuthSession(request.session)
            equip_script = auth.get_clientFactory(
            ...
            if have_errors == False:
                messages.add_message(
                    request, messages.SUCCESS, equip_script_messages.get("success_remove"))
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    client.create_equipamento_roteiro().inserir(
                        equipment, id_script)
                    messages.add_message(
                        request, messages.SUCCESS, equip_script_messages.get("success_insert"))
                except EquipamentoRoteiroError, e:
                    messages.add_message(
                        request, messages.ERROR, equip_script_messages.get("error_equal_ass"))
