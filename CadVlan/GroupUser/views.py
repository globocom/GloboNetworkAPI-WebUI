        ...
        if form.is_valid():
            ...
            # If all has ben removed
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    group_user_messages.get("success_remove"))
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        group_user_messages.get("success_insert"))
    ...
    try:
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    messages.add_message(
                        request,
                        messages.SUCCESS,
                        group_user_messages.get("success_edit"))
                    return redirect('group-user.list')
        ...
        if group_user is None:
            messages.add_message(
                request,
                messages.ERROR,
                group_user_messages.get("invalid_group_user"))
            return redirect('group-user.list')
        ...
        # Set Group User data
        initial = {
            'id_group_user': group_user.get('id'),
            'name': group_user.get('nome'),
            'read': get_permission(
                group_user.get('leitura')),
            'write': get_permission(
                group_user.get('escrita')),
            'edition': get_permission(
                group_user.get('edicao')),
            'delete': get_permission(
                group_user.get('exclusao'))}
