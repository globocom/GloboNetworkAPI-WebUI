...
class AclForm(forms.Form):
    ...
    acl = forms.CharField(label=u'ACL',
                          required=False,
                          error_messages=error_messages,
                          widget=forms.Textarea(
                              attrs={'style': "width: 800px;height:400px;resize:none;"}))
    comments = forms.CharField(label=u'Comentários',
                               required=False,
                               error_messages=error_messages,
                               widget=forms.TextInput(
                                   attrs={'style': "width: 300px"}))
    apply_acl = forms.BooleanField(widget=forms.HiddenInput(),
                                   label='',
                                   required=False)
...
class TemplateForm(forms.Form):
    ...
    name = forms.CharField(label=u'Nome do Template',
                           required=True,
                           error_messages=error_messages,
                           widget=forms.TextInput(
                               attrs={'style': 'width: 350px'}))
    content = forms.CharField(label=u'Template',
                              required=True,
                              error_messages=error_messages,
                              widget=forms.Textarea(
...
class TemplateAddForm(forms.Form):
    ...
    name_ipv6 = forms.CharField(label=u'Nome do Template',
                                required=False,
                                error_messages=error_messages,
                                widget=forms.TextInput(
                                    attrs={'style': 'width: 350px'}))
    content_ipv6 = forms.CharField(label=u'Template',
                                   required=False,
                                   error_messages=error_messages,
                                   widget=forms.Textarea(
                                       attrs={'style': "width: 600px;height:310px;resize:none;"}))
    ...
    def clean_name_ipv6(self):
        ...
        if self.cleaned_data['name_ipv6'] and ' ' in self.cleaned_data['name_ipv6']:
            raise forms.ValidationError(
                'O nome do template não deve conter espaços')
        elif self.cleaned_data['name_ipv6'] and not check_regex(
                self.cleaned_data['name_ipv6'], "^[a-zA-Z0-9_.-]+$"):
            raise forms.ValidationError(
                'Nome do template contém caracteres inválidos.')
