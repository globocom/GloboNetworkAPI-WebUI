...
def create(request, id_vlan, network):
    ...
    except (NetworkAPIClientError, CVSError, ValueError) as e:
        logger.error(e)
...
def edit(request, id_vlan, network):
    ...
    try:
        ...
        vlan['network'] = replace_id_to_name(
            list_ips,
            client.create_tipo_rede().listar().get('net_type'),
            "network_type",
            "id",
            "name")
...
def validate(request, id_vlan, network):
    try:
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            acl_messages.get("success_validate") %
            network)
                ...
                if name_ipv4 and content_ipv4:
                    if check_template(name_ipv4, IP_VERSION.IPv4[1], user):
                        ...
                        messages.add_message(
                            request,
                            messages.ERROR,
                            acl_messages.get("field_duplicated") %
                            name_ipv4)
                    else:
                        create_template(
                            name_ipv4,
