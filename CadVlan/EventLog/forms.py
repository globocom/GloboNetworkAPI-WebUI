...
class SearchFormLog(forms.Form):
    ...
    user = forms.ChoiceField(
        label=u'Usuário', required=False, choices=[
            ('', 'Selecione')], widget=forms.Select(
            attrs={
                'style': "width: 300px;"}), error_messages=error_messages)
    ...
    def clean(self):
        ...
        if first_date != '' and last_date == '':
            raise forms.ValidationError(
                "A data final do período deve ser informada.")
        if first_date == '' and last_date != '':
            raise forms.ValidationError(
                "A data inicial do período deve ser informada.")
