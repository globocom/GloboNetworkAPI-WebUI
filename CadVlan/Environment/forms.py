...
class DivisaoDCForm(forms.Form):
    ...
    nome = forms.CharField(
        label=u'Divisão do Data Center',
        min_length=2,
        max_length=100,
        required=True,
        error_messages=error_messages,
...
class AmbienteForm(forms.Form):
    ...
    def __init__(
        ...
        self.fields['ambiente_logico'].choices = [
            (amb_log['id'],
             amb_log['nome']) for amb_log in env_logic["logical_environment"]]
    ...
    divisao = forms.ChoiceField(
        label="Divisão DC", choices=[
            (0, 'Selecione')], required=True, widget=forms.Select(
            attrs={
                'style': "width: 160px"}), error_messages=error_messages)
    ambiente_logico = forms.ChoiceField(
        label="Ambiente Lógico", required=True, choices=[
            (0, 'Selecione')], widget=forms.Select(
            attrs={
                'style': "width: 210px"}), error_messages=error_messages)
    grupol3 = forms.ChoiceField(
        label="Grupo Layer3", choices=[
            (0, 'Selecione')], required=True, widget=forms.Select(
            attrs={
                'style': "width: 280px"}), error_messages=error_messages)
    ...
    def clean_min_num_vlan_1(self):
        ...
        if (
            max_num_vlan_1 != '' and min_num_vlan_1 == '') or (
            min_num_vlan_1 != '' and max_num_vlan_1 == '') or (
            not max_num_vlan_1 and min_num_vlan_1) or (
                not min_num_vlan_1 and max_num_vlan_1):
    ...
    def clean_min_num_vlan_2(self):
        ...
        if (
            max_num_vlan_2 != '' and min_num_vlan_2 == '') or (
            min_num_vlan_2 != '' and max_num_vlan_2 == '') or (
            not max_num_vlan_2 and min_num_vlan_2) or (
                not min_num_vlan_2 and max_num_vlan_2):
            raise forms.ValidationError(
                'O valor máximo e mínimo devem ser preenchidos.')
        ...
        if not self._errors:
            ...
            if ip_version == "v4":
                ...
                self.cleaned_data['network_validate'] = v6oct1 + ':' + v6oct2 + ':' + v6oct3 + ':' + \
                    v6oct4 + ':' + v6oct5 + ':' + v6oct6 + ':' + v6oct7 + ':' + v6oct8 + '/' + v6oct9
